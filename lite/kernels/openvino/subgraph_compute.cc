...
#include "lite/kernels/openvino/subgraph_compute.h"
#include <sys/time.h>
#include <time.h>
#include <limits>
#include <utility>
...
void AddFeed(cpp::ProgramDesc& desc, Scope& scope, const std::string& in_name, int col = 0)
{
  auto* block = desc.GetBlock<cpp::BlockDesc>(0);
...
void AddFetch(cpp::ProgramDesc& desc, Scope& scope, const std::string& in_name, int col = 0)
{
  auto* block = desc.GetBlock<cpp::BlockDesc>(0);
  auto* op = block->AddOp<cpp::OpDesc>();
...
bool SubgraphEngine::BuildDeviceProgram() {
  ...
  for (auto& inst : insts) {
    ...
    // if (!bridges.Exists(op_type, TARGET(kOV))) {
    //   return false;
    // }
  ...
  // xxx
  {
    ...
    {
      AddFeed(*new_program, *exec_scope_, input_names_[i]);
    }
    for (size_t i = 0; i < output_names_.size(); i++)
    {
      AddFetch(*new_program, *exec_scope_, output_names_[i]);
    }
    SaveModelPb("xxx", *exec_scope_, *new_program, true);
...
REGISTER_LITE_KERNEL(subgraph,
                     ...
                     def)
    .BindInput("Inputs",
               {LiteType::GetTensorTy(TARGET(kOV), PRECISION(kFloat))})
    .BindOutput("Outputs",
                {LiteType::GetTensorTy(TARGET(kOV), PRECISION(kFloat))})
    .Finalize();
